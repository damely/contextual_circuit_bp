...
def auxilliary_variables():
    ...
    return {
        ...
        'recurrent_nl': tf.nn.leaky_relu,  # tf.nn.leakyrelu, tf.nn.relu, tf.nn.selu
...
class ContextualCircuit(object):
    ...
    def input_integration(self, P, U, I, O, I_update):
        """Steady state Integration on the input."""
        numerator = (
            (self.xi * self.X) -
            (self.mu * U) -
            (self.nu * P))
        denominator = (
            (self.alpha * U) +
            (self.beta * P)) + 1e-12
        I_summand = self.recurrent_nl(numerator / denominator)
        if not self.gru_gates:
            # Alternatively, forget gates on the input
            return (I_update * I) + ((1 - I_update) * I_summand)
        else:
    ...
    def output_integration(self, P, Q, I, O, O_update):
        ...
        if self.multiplicative_excitation:
            ...
            # Additive gating I + P + Q
            O_summand = self.recurrent_nl(self.gamma * P + self.delta * Q)
