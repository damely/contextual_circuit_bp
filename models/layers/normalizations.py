...
class normalizations(object):
    ...
    def contextual_single_ecrf_simple(self, x, layer, eRF, aux):
        """Contextual model from paper with frozen U & eCRFs."""
        self.update_params(aux)
        self.set_RFs(layer=layer, eRF=eRF)
    ...
    def old_cc(self, x, layer, eRF, aux):
        """Contextual model from paper with frozen U & eCRFs."""
        self.update_params(aux)
        self.set_RFs(layer=layer, eRF=eRF)
        contextual_layer = old_cc.ContextualCircuit(
            X=x,
            timesteps=self.timesteps,
            SRF=self.SRF,
            SSN=self.SSN,
    ...
    def baseline_context(self, x, layer, eRF, aux):
        """Contextual model from paper with frozen U & eCRFs."""
        self.update_params(aux)
        self.set_RFs(layer=layer, eRF=eRF)
        contextual_layer = baseline_context.ContextualCircuit(
            X=x,
            timesteps=self.timesteps,
            SRF=self.SRF,
            SSN=self.SSN,
    ...
    def contextual_single_ecrf_time(self, x, layer, eRF, aux):
        """Contextual model from paper with frozen U & eCRFs."""
        self.update_params(aux)
        self.set_RFs(layer=layer, eRF=eRF)
        contextual_layer = contextual_single_ecrf_time.ContextualCircuit(
            X=x,
            timesteps=self.timesteps,
            SRF=self.SRF,
            SSN=self.SSN,
