...
if 'confusion_matrix' in targets.items()[0][1]['metrics']:
    num_models = len(models)
    # f = plt.figure()
    # for idx, (k, v) in enumerate(results.iteritems()):
    #     plt.subplot(1, num_models, idx + 1)
    #     plt.title(models[k][1])
    #     plt.imshow(v[0]['confusion_matrix'])
    # plt.show()
    model_names = [models[k][1] for k in selected_scores.keys()]
    cms = np.concatenate(
        [v[0][None, :] for v in selected_scores.values()],
            axis=0)
    df = pd.DataFrame(cms.transpose(), columns=model_names)
